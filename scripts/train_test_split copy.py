...
class SinogramDataset(Dataset):
    def __init__(self, clean_root: Path, art_root: Path, patch: int = 256):
        ...
        # artifact files with SAME NAME
        self.art_paths = []
        for p in self.clean_paths:
            art_file = self.art_root / p.name
            if not art_file.exists():
                raise RuntimeError(f"Artifact file missing: {art_file}")
            self.art_paths.append(art_file)

        self.patch = int(patch)

        if len(self.clean_paths) == 0:
            raise RuntimeError(f"No clean .npy files found in {self.clean_root}")
...
def main():
   ...
   # criterion = nn.MSELoss(reduction='mean')
    criterion = L1SSIMLoss(alpha=0.75) #changed from 0.84 to 0.7 for more pixel focus
    ...
    with torch.no_grad():
        ...
        #for art, clean in test_loader:
            ...
            if batch_idx == 0:
                ...
                save_image(art[0], art_sino_dir / f"epoch{epoch:03d}_input.png")
                save_image(pred[0], recon_sino_dir / f"epoch{epoch:03d}_recon.png")
                save_image(clean[0], gt_sino_dir / f"epoch{epoch:03d}_target.png")

                print(f"Saved reconstructed sinograms to {recon_sino_dir}")
